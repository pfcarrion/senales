...
@login_required
def senales_trading(request):
    # Verifica si el usuario tiene una suscripción activa
    suscripciones_activas = Subscription.objects.filter(usuario=request.user, estado='pagado')
    if not suscripciones_activas.exists():
        return render(request, 'core/acceso_denegado.html', {
            "mensaje": "Necesitas una suscripción activa para ver las señales."
        })

    # Cargar señales del archivo JSON generado por el bot
    senales_json_path = "/opt/senales/static/senales/senales.json"
    senales = []
    if os.path.exists(senales_json_path):
        with open(senales_json_path, "r", encoding="utf-8") as file:
            senales = json.load(file)

    return render(request, 'core/senales.html', {"senales": senales})
